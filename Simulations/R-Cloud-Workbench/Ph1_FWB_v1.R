...
run.2 <- function(code) {
  ...
  d3 <- vTrip.fast.1(s[s$strata == 3, c("int.ID", ".week", "coord_x_p", "coord_y_p")], frame.int[c("int.ID", "x_int", "y_int")])
  d4 <- vTrip.fast.1(s[s$strata == 4, c("int.ID", ".week", "coord_x_p", "coord_y_p")], frame.int[c("int.ID", "x_int", "y_int")])
  d5 <- vTrip.fast.1(s[s$strata >= 3, c("int.ID", ".week", "coord_x_p", "coord_y_p")], frame.int[c("int.ID", "x_int", "y_int")])
...
arg.2 <- data.frame(code = c(des1, des2), stringsAsFactors = F)
...
test <- Sim(fun = "run.2", arg = arg.1, I = I, print = F, log = F)
